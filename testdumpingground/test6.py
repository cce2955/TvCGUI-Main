...
# Integrated: character-agnostic CSV mapping (move_id_map_charagnostic.csv)
...
# Character-agnostic mapping (CSV editable by you)
...
def load_charagnostic_csv(path=CHAR_AGNOSTIC_CSV):
    ...
    try:
        if not os.path.exists(path):
            ...
            return mapping
        # Expect columns: atk_id_dec, atk_id_hex, generic_label, top_label, examples
...
def read_attack_ids(base):
    """Read primary and secondary attacker id offsets from a fighter base."""
...
def main():
    ...
    if hit_new:
        hitw.writerow(["t","victim_label","victim_char","dmg","hp_before","hp_after",
                       "team_guess","attacker_label","attacker_char","attacker_id_dec","attacker_id_hex","attacker_move","dist2","victim_ptr_hex"])
                ...
                if took and dmg and dmg >= MIN_HIT_DAMAGE:
                    ...
                    if SHOW_HITS:
                        print(f"[{t:10.3f}] HIT  victim={name:<5}({fi['name']:<16}) "
                              ...
                              f"atk_id={(atk_id or '--')} atk_sub={(atk_sub or '--')} atk_name={(atk_name or '--')}")

                    # CSV row (append attack id + name)
