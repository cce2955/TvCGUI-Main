...
# Full HUD poller with character names, meter, position, and "last damage chunk" (+0x40).
...
OFF_CUR_HP     = 0x28
OFF_AUX_HP     = 0x2C
OFF_LAST_HIT   = 0x40  # Observed: "last damage chunk" (on victim)
...
# Misc (unused but kept for future)
OFF_ANIM_STATE   = 0x58
OFF_ANIM_FLAG    = 0x5C
OFF_ACTION_STATE = 0x60
OFF_SUB_ACTION   = 0x64
OFF_HIT_STATE    = 0x00
OFF_TEAM_FLAG    = 0x30
OFF_FRAME_COUNTER= 0x6C
...
class MeterAddrCache:
    ...
    def get(self, base):
        ...
        for a in cands:
            v = rd32(a)
            if v in (50000, 0x0000C350):  # common init
...
def read_block(base, posy_off, want_meter=True):
    ...
    char_name = CHAR_NAMES.get(char_id, f"Unknown_ID_{char_id}") if char_id is not None else "???"

    # Position
    x = rdf32(base + POSX_OFF)
    y = rdf32(base + posy_off) if posy_off is not None else None

    # Last damage chunk (on victim)
    last_hit = rd32(base + OFF_LAST_HIT)
    if last_hit is None or last_hit < 0 or last_hit > 200000:
        last_hit = None
    ...
    pkt = {
        ...
        "x": x, "y": y,
        "last_hit": last_hit,  # <- NEW: last damage chunk received (ΔMissing of most recent hit)
    }
...
def fmt(label, blk):
    ...
    if pct_val is not None:
        if pct_val > 0.66: hp_color = Colors.GREEN
        elif pct_val > 0.33: hp_color = Colors.YELLOW
    ...
    # Last damage chunk (received)
    if blk.get("last_hit") is not None and blk["last_hit"] > 0:
        dmg_str = f" | lastDmg:{blk['last_hit']:5d}"
    else:
        dmg_str = f" | {Colors.DIM}lastDmg:--{Colors.RESET}"

    return f"{label_color}{label}{Colors.RESET}[{Colors.DIM}{blk['ptr']:08X}{Colors.RESET}]{char} {hp_display} {pct_str}{m}{x}{y}{dmg_str}"

# ---------------- Main ----------------
def main():
    print(f"{Colors.BOLD}{'='*80}")
    print("TvC HUD POLLER WITH CHARACTER NAMES + LAST DAMAGE CHUNK (+0x40)")
    ...
    print(f"\n{Colors.BOLD}{'='*80}")
    print("Polling HP/Meter/X/Y/Character/lastDmg… (Ctrl+C to quit)")
    print(f"{'='*80}{Colors.RESET}")
    print(f"{Colors.P1_BRIGHT}P1 = Blue{Colors.RESET} | {Colors.P2_BRIGHT}P2 = Red{Colors.RESET} | {Colors.GREEN}HP: Green>Yellow>Red{Colors.RESET} | {Colors.PURPLE}Meter = Purple{Colors.RESET} | lastDmg = last damage received (+0x40)")
