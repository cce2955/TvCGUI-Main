...
from .dialogs import ReplaceMoveDialog
...
from .widgets import ManualAnimIDDialog, get_field_help, ask_integer_with_help, ask_float_with_help, ask_hit_result_flags_with_presets, apply_titlebar_icon, configure_light_dialog, build_dialog_shell, make_list_picker, finalize_dialog_geometry
...
class FDCellEditorsMixin:
    ...
    def _resolve_anim_binding(self, mv):
        try:
            from tvcgui.platform.dolphin import rbytes
            from tvcgui.features.animation.runtime import get_current_animation_id, locate_loaded_mot
        except ImportError:
    ...
    def _write_anim_id(self, mv, new_anim_id: int) -> bool:
        if not U.WRITER_AVAILABLE:
            return False
        try:
            from tvcgui.platform.dolphin import rbytes, wd32
            from tvcgui.features.animation.runtime import write_animation_only
        except ImportError:
            return False
        char_name, char_id = self._animation_context(mv)
