...
def _find_csv_path():
    ...
    candidates = [
        data_path("combat", "move_id_map_charagnostic.csv"),
        os.path.join(os.getcwd(), "data", "combat", "move_id_map_charagnostic.csv"),
        # Legacy working-directory fallback for an older portable install.
        os.path.join(os.getcwd(), "move_id_map_charagnostic.csv"),
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
