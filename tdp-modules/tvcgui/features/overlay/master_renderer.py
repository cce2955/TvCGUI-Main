...
class MasterOverlay:
    ...
    def _draw_active_mission_panel(
        ...
        row_gap = 4
        visible_limit = 6
        panel_w = max(560, min(int(self.w * 0.48), 820))
        ...
        header_h = max(36, title_h + 8)
        challenge_h = 56
        row_h = max(30, self.smallfont.get_height() + 12)
        timer_h = 44 if (
        ...
        footer_h = 21 if collapsed_scroll else 8
        panel_h = pad + header_h + 7 + challenge_h + 8 + timer_h + note_h + list_h + footer_h + pad
        ...
        panel.blit(label_surf, (challenge_rect.x + 10, challenge_rect.y + 6))

        button_y = challenge_rect.y + 12
        ...
        button_widths = [max(82, self.smallfont.size(text)[0] + 20) for text, _kind in button_specs]
        button_x = challenge_rect.right - 8 - sum(button_widths) - 7
        ...
        detail_left = challenge_rect.x + 10
        ...
        panel.blit(detail_surf, (detail_left, challenge_rect.y + 27))

        for (text, kind), button_w in zip(button_specs, button_widths):
            rect = pygame.Rect(button_x, button_y, button_w, 28)
