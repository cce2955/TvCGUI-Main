...
def _patch_bool_enabled(value: Any) -> bool:
    try:
        from .patterns import SUPERBG_ON
    except Exception:
        SUPERBG_ON = 0x04
...
def _ensure_speed_mod(mv: dict) -> bool:
    ...
    try:
        ...
        from .patterns import find_speed_mod_addr
...
def _ensure_attack_property(mv: dict) -> bool:
    ...
    try:
        from tvcgui.platform.dolphin import rbytes
        from .patterns import find_attack_property_addr
...
def _ensure_superbg(mv: dict) -> bool:
    ...
    try:
        ...
        from .patterns import find_superbg_addr
...
def _ensure_combo_kb_mod(mv: dict) -> bool:
    ...
    try:
        from tvcgui.platform.dolphin import rbytes
        from .patterns import find_combo_kb_mod_addr
...
def apply_patch_value_to_move(mv: dict, entry: dict) -> None:
    ...
    try:
        if group == "move":
            ...
            try:
                from .patterns import SUPERBG_ON, SUPERBG_OFF
            except Exception:
