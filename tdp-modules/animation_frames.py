...
def _candidate_paths() -> Iterable[Path]:
    ...
    if override:
        yield Path(override)
    yield Path(__file__).resolve().with_name("animation_frames.json")
    # Frozen builds can leave Python modules in a bundle directory while the
    # data file lives next to the executable. Keep this fallback harmless for
    # source runs.
    try:
        import sys
        if getattr(sys, "frozen", False):
            yield Path(sys.executable).resolve().with_name("animation_frames.json")
    except Exception:
        pass
